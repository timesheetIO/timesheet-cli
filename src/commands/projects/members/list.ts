...
import type { ProjectMember, ProjectMemberListParams } from '@timesheet/sdk';

export function registerProjectMembersListCommand(parent: Command): void {
  parent
    .command('list')
    ...
    .action(
      async (
        ...
        spinner.start('Loading project members...');

        const limit = parseInt(options.limit, 10) || getConfig('paginationLimit');
        const params: ProjectMemberListParams = { limit, projectId };
        if (options.status) params.status = options.status;
        if (options.withoutMe) params.withoutMe = true;
        if (options.includeDeleted) params.withDeleted = true;
        if (options.search) params.search = options.search;

        const page = await client.projects.listMembers(projectId, params);
        spinner.stop();
        ...
        if (page.items.length === 0) {
          output(formatter, formatter.formatInfo('No project members found.'));
          return;
        }
